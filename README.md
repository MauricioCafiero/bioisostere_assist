...
library, and the resulting analogue built by grafting each hit onto
rosuvastatin's own core (real output from the run above):
...
similarity (`examples/rosuvastatin_matches.png`, generated by the run above):

![Top-12 shape-bioisosteric rosuvastatin analogues, each labeled with its similarity score and the fragment swap that produced it](examples/rosuvastatin_matches.png)
